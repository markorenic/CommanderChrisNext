...
fn analyze_command_safety(cmd: &str) -> (bool, Vec<&str>) {
    let dangerous_commands = ["rm", "sudo", "mv", "dd", ">", "mkfs", "chmod", "chown", "kill"];
    let side_effects = ["write", "create", "delete", "modify", "install", "uninstall", "download"];
...
pub fn prompt_yes_no(prompt: &str, default: bool) -> Result<bool> {
    ...
    match input.as_str() {
        ...
        "n" | "no" => Ok(false),
        _ => {
            println!("Invalid input, using default: {}", if default { "yes" } else { "no" });
            Ok(default)
        }
    }
}
    ...
    for line in response.lines() {
        if line.starts_with("```") {
            if !in_code_block {
                ...
                // End of code block
                in_code_block = false;
                if code_block_lang.contains("sh") || code_block_lang.contains("bash") || code_block_lang.contains("shell") {
